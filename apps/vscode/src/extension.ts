...
class NyxaraView implements vscode.TreeDataProvider<any> {
  ...
  getChildren(): any[] {
    ...
    const rows: any[] = [new NyxaraItem("Prompt: Use ‘Nyxara: Generate Plan’", undefined, { command: "nyxara.generatePlan", title: "Generate" })];
    if (!plan) return rows;
    ...
    if (snapshot) {
      ...
      if (snapshot.status === "awaiting_plan_approval") rows.push(new NyxaraItem("Approve & Run", undefined, { command: "nyxara.approveAndRun", title: "Approve" }));
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const session = new NyxaraSession(context, output, setting("nyxara.openaiCompatible.baseUrl") || "https://api.openai.com/v1");
